...
- **Migration and SQL function debugging.** Shipped fixes for migration ordering and SQL function evolution: dropping a function before recreating with a new return type, switching from `uuid_generate_v4()` to `gen_random_uuid()` in an auth_errors migration, adding a missing `created_by` column in a course-template copy function. Forward-compatible patches, never rewriting history.
